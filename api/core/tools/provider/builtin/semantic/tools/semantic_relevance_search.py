...
class SemanticRelevanceSearchTool(BuiltinTool):
    ...
    def query(self, query: str, fields_of_study: str, year: str, fields: str, num_results: int = 50) -> ToolInvokeMessage | list[
        ToolInvokeMessage]:
        """
        ...
        if total == 0:
            ...
            if total <= self.max_results or total <= num_results:
                # the total number of results is less than self.max_results, return all results
                return self.create_text_message(json.dumps(data))
            else:
                # the total number of results is more than self.max_results, and num_results is more than self.max_results
                result = data
                # rest number of results to get
                rest_num_results = num_results - self.max_results
                # offset to get the rest of the results
                offset = limit
                for i in range(0, rest_num_results // self.max_results + 1):
                    # use the offset to get the rest of the results
                    total, data = self.query_once(query, fields_of_study, year, fields, offset=offset, limit=min(rest_num_results, self.max_results))
                    result.extend(data)

                    # update the offset and rest_num_results
                    offset += self.max_results
                    rest_num_results -= self.max_results

                    if rest_num_results <= 0 or offset >= num_results:
                        break
