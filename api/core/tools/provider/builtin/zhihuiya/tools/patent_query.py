...
class PatentQueryAPI:
    ...
    """
    api_key = ""
    api_url = "https://connect.zhihuiya.com/search/patent/query-search-patent"
    max_limit = 100
    max_offset = 1000
    interval = 1
    query_template = "TACD:{}"
    ...
    def query_once(self, query: str, limit: int = 50, offset: int = 0, sort: List[dict] = None,
    ...
    def query(self, query: str, num: int = 50, sort: List[dict] = None,
        ...
        offset = 0
        query = query.strip()
        total, result = self.query_once(query, limit, offset, sort, collapse_type, collapse_by, collapse_order)

        rest_num = min(total, num) - limit
        while rest_num > 0:
            limit = min(rest_num, self.max_limit)
            offset = len(result)
            total, data = self.query_once(query, limit, offset, sort, collapse_type, collapse_by, collapse_order)
            result.extend(data)
            rest_num -= limit
...
class PatentQueryTool(BuiltinTool):
    def _invoke(self,
        ...
        results = PatentQueryAPI(self.runtime.credentials).query(query, num, sort, collapse_type, collapse_by, collapse_order)
